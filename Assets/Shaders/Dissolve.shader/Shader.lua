{
    ...
    properties =
    {
        ...
        _edgeColor = { "vec4", {0.0, 0.27863919734955, 1.0, 1.0} },
        _edgeGlow = { "float", 20.0 },
        _edgeWidth = { "float", 0.029999999329448 },
        _time = { "float", "0.0" },
        _gradient = { "texture2D", "Gradient", { repeats = false, mipmap = false } },
    ...
    pass =
    {
        ...
        [[
            ...
            void surface(in Input IN, inout SurfaceOutput o)
            {
                ...
                o.roughness = 0.32237;
                o.metalness = 1.0;
                float localVar_Progression = remap(sin(_time), vec2(-1.0, 1.0).x, vec2(-1.0, 1.0).y, vec2(0.0, 1.0).x, vec2(0.0, 1.0).y);
                float temp_3 = turb_snoise3D((IN.worldPosition*vec3(_noiseScale, _noiseScale, _noiseScale)), 2, 3.000000, 0.500000);
                vec3 multiply_11 = (texture(_gradient, vec2(remap(IN.worldPosition, vec2(0.0, 2.0).x, vec2(0.0, 2.0).y, vec2(0.0, 1.0).x, vec2(0.0, 1.0).y).g, 0.0)).rgb * remap(temp_3, vec2(-1.0, 1.0).x, vec2(-1.0, 1.0).y, vec2(0.0, 1.0).x, vec2(0.0, 1.0).y));
                o.emission = ((_edgeColor.rgb*vec3(_edgeGlow, _edgeGlow, _edgeGlow)) * (1.0 - step((_edgeWidth+localVar_Progression), multiply_11)));
                o.opacity = multiply_11.r;
                o.occlusion = 1.0;
